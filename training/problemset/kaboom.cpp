...
int main(){ _
	...
	for(int i = 0; i < n; i++) cin >> car[i] >> speed[i];
	double ans = DINF;
	for(int i = 0; i < n; i++){
		for(int j = 0; j < i; j++){
			pt crash = inter(car[i], car[j]);
			if(crash == pt(DINF,DINF)) continue;
			double dist1 = dist(car[i].p, crash);
			double dist2 = dist(car[j].p, crash);
			double t = max(dist1/speed[i], dist2/speed[j]);
			ans = min(ans,t);
		}
	}

	if(ans == DINF) cout << "No show :(" << endl;
