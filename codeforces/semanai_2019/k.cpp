...
const ll LINF = 0x3f3f3f3f3f3f3f3fll;
	...
	struct edge {
		...
		edge() : to(-1), rev(0), flow(0), cap(0), cost(0), res(false) {}
...
int main(){ _
	...
	vector<int> A, B;
	for (int i = 0; i<n; i++) {
		if (dev[i] > 0) A.push_back(i);
		if (dev[i] < 0) B.push_back(i);
	}
	...
	int sp = false;
	if (A.size() < B.size()) swap(A, B), sp = true;
	
	mcmf<int> G(A.size()+B.size()+2);
	int s = A.size() + B.size();
	int t = s+1;

	for (int i : A) {
		G.add(s, i, abs(dev[i]), 0);
	}
	for (int i : A) {
		G.add(i, t, abs(dev[i]), 0);
	}
	for (int i : A) for (int j : B) G.add(i, j, INF, 1);
	
	auto solve = G.min_cost_flow(s, t);
	
	set<int> sa, sb;
	for (auto u : A) sa.insert(u);
	for (auto u : B) sb.insert(u);
	vector<tuple<int, int, int>> ans;
	for (int i : A) for (edge e : g[i]) {
		if (sb.count(e.to) and e.flow > 0) {
